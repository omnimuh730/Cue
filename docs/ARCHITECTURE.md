...
Streaming is paced, not per token: deltas are batched into the model at 25 fps and SwiftData is committed at most once a second while a turn runs (and at turn end). `ChatView` iterates `Message` objects, so a delta re-renders only its bubble. Assistant Markdown is split by `MarkdownBlocks` (`Shared/`) into paragraph, heading, list, block quote, pipe table, rule, code, and Mermaid blocks; `MarkdownRenderer` parses inline Markdown off the main thread and reuses already-parsed blocks, so a streaming update costs one trailing block. A table only counts once its `|---|` separator has arrived, so a table being streamed does not flicker between prose and grid. A line that opens and closes a backtick span inline (```` ```text``` means plain output ````) is prose, not a fence: CommonMark forbids a backtick in a fence's info string (`MarkdownBlocks.fenceInfo`), and treating such a line as a fence opened a block that never closed and swallowed the rest of the answer.

A bubble draws the previous pass only while its turn is streaming (`MarkdownDocumentChoice`): the next flush is 40 ms away and corrects it. Once the turn has ended there is no next flush, so a parse that has fallen behind is redone inline instead of leaving the reader with an answer that stops mid-sentence or a code block frozen on the characters that had arrived. The parse is keyed on the text, the diagram mode, and whether the turn is live, so toggling Mermaid mode or a turn ending always reparses; and the reveal head never rewinds onto a parse that has not landed yet, which would blank text mid-read.
...
Forking a chat is `git checkout -b`. `AppSession.fork(from:quoting:)` copies the turns up to and including a message (`TurnTruncation.keep(inclusive:)`, the rule regenerate and delete already use) into a new `Conversation` that records `forkedFromID` and `forkedAtMessageID`; the source thread is never touched. Copies are new `Message` rows with new ids, so the two branches diverge from the first edit — `responseID` rides along, so a Personal branch chains from the same OpenAI response, while `codexThreadID` is dropped because the Codex thread no longer matches the turns.

`ChatBranching` (`Shared/`) holds the tree math, pure: `rootID` walks the fork links up (a broken or cyclic link stops the walk rather than looping), `family` returns a chat's branches with `main` first and the forks in creation order, `children` finds what was forked out of a branch, and `quote` / `appendingQuote` build the Markdown block quote the composer receives. `AppSession` keeps `conversationsByAge` beside the sidebar order so the tab bar never sorts per row.

`BranchTabBar` sits in `ChatView`'s top safe-area inset, under the toolbar, and only when the chat has more than one branch. `ForkTrailView` marks the other half: the turn a branch was cut at carries a chip per branch, and opening one draws that branch's own turns under a rail as a thread, any row of which opens the branch on that turn (`openBranch(_:at:)` sets `scrollTarget`). The transcript works the split points out once per pass from `forksByMessage` rather than searching the family per row, and a branch's thread is `ChatBranching.continuation` — the turns newer than the fork itself, since an inherited turn is a copy that keeps the time of the turn it came from. The sidebar lists families rather than chats (`sidebarConversations`), carries a branch count, and `selectFamily` reopens the branch that family was last read in (`activeBranches`). `deleteBranch` soft-deletes one fork and reattaches its children to its parent, with Undo restoring both the branch and the links; `deleteConversation` takes the whole family. ⌘[ / ⌘] move between threads, ⌥⌘[ / ⌥⌘] between branches of the one on screen.

Selecting text in a message floats `SelectionActionsOverlay` over the passage. `SelectableNSTextView` reports the selection on mouse-up with its rectangle converted to panel content space (origin top-left, the space the virtual cursor uses) through the `selectMessageText` environment closure, which `ChatView` binds per bubble so the actions know which message the passage came from. **Add to chat** quotes it into the composer; **Fork** branches at that message and seeds the new branch's draft with the quote. The overlay closes on Escape, on a scroll, and when the chat changes.
